...
| R-3 | ผู้ใช้ทุกคนใน tenant ที่มี role `owner` ผ่านด่านหมด ไม่ใช่แค่ owner คนที่ team lead คิดถึง | ✅ **ตรวจแล้ว 2026-08-18 (เจ้าของยืนยัน): ฝั่ง dev/test มี owner บัญชีเดียว** จึงไม่มีสิทธิ์เกินที่ตั้งใจในตอนนี้ · **ต้องตรวจซ้ำก่อนเปิดใช้บน production** เพราะเป็นคนละฐานข้อมูล · ถ้าวันหนึ่งมี owner เพิ่มโดยไม่ตั้งใจ ให้แก้ role ไม่ใช่แก้โค้ด |
...
7.1. ตั้ง `UPLOADER_URL` ให้มี path เกินมาชั่วคราว → ได้ `502` **ไม่ใช่ `500`**
   (uploader คืน `200` + HTML ในกรณีนี้ ยืนยันจากการยิงจริง 2026-08-18)
