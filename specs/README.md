...
| 15 | [15-spec-openclaw-migration-tickets-plan.md](./15-spec-openclaw-migration-tickets-plan.md) | **ACCEPTED (2026-07-23), 3 audit rounds, round 3 GO.** Pre-ticket plan turning spec 14's six work items into tickets T10–T16 (IDs, file scope, dependency order); adds the one piece of context spec 14 didn't have — this machine's `~/.openclaw/` install is a local test sandbox, not production (which runs on Linux/Docker on a separate host/repo) |
| 16 | [16-spec-openclaw-consolidated-skill.md](./16-spec-openclaw-consolidated-skill.md) | **DRAFT (2026-07-24) — not yet audited.** Supersedes spec 14 § 3.1 only: collapses `login`/`setup`/`logout` into one invocable `bobby-cli` skill (registers `/bobby_cli`) instead of four separate skills; renames `setup-memory` → `setup-chawengburi`; archives the superseded originals into `old_skills/` |
| 17 | [17-spec-uploader-document-search.md](./17-spec-uploader-document-search.md) | 🔴 **PARTIALLY SUPERSEDED by spec 18 draft 3 — its auth/transport half is dead; its command-behavior half (§ 3.1, § 3.4, § 5.3, § 6) is still the contract.** **DRAFT (2026-08-11, revised after review) — urgent demo slice, throwaway by design.** `uploader login`/`search`/`fetch` over `GET /api/ai/search-files` + one additive `AGENTS.md` section so the Discord bot answers "ขอดูไฟล์ reservation หน่อย" with **file name + description**, then from the markdown body. One shared PocketBase token on the host (env vars only, no `--token`/`--base-url`); auth-center and the memory domain untouched on purpose. openClaw baseline snapshotted + tagged `pre-uploader-2026-08-11` before any edit (§ 5.1) |

| 18 | [18-spec-uploader-auth.md](./18-spec-uploader-auth.md) | **DRAFT (2026-08-17), third draft — rewritten whole; § 10 records why it reverses draft 2's "don't use auth-center".** Supersedes spec 17's auth/transport sections entirely. auth-center becomes an authorizing proxy: it holds the single long-lived uploader token in its own env, verifies the caller's `sm_live_` token against the DB and requires the `owner` role, then calls the uploader itself — bobby-cli never sees the uploader token, gains **zero** new env vars or credential files, and the rule sits behind the HTTP boundary so any agent (not just openClaw) is bound by it. The uploader itself is still untouched. § 5.5 is the section to not get wrong: a dead uploader token must surface as `502`, never `401`. § 9 lists the triggers that force a redesign — guild use is the live one |

**Read order:** 01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09 → 10 → 11 → 12 → 13 → 14 → 15 → 16 → 17 → 18
